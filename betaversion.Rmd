...
ggplot(data = combined_data, aes(x = Year, y = Unemp_Rate)) +
  ...
  theme_minimal() +
  labs(title = "Unemployment Rate in the Netherlands (2015-2025)", x = "Year", y = "Unemployment Rate (%)") +
  ylim(2, 8.5)
```
...
ggplot(data = combined_data) +
  ...
  scale_x_continuous(breaks = seq(2015, 2025, by = 2)) +
  labs(title = "Youth vs. Total Unemployment in the Netherlands", x = "Year", y = "Unemployment Rate (%)", color = "Demographic Group")
```
...
ggplot(data = combined_data, aes(x = GDP_Growth, y = Unemp_Rate)) +
  geom_smooth(data = filter(combined_data, Year != 2020), 
              method = "lm", color = "gray40", linetype = "dashed", se = FALSE, fullrange = TRUE) +
  geom_point(data = filter(combined_data, Year != 2020), 
             size = 4, color = "royalblue") +
  geom_point(data = filter(combined_data, Year == 2020), 
             size = 5, color = "firebrick") +
  geom_text(aes(label = Year), vjust = -1.3, size = 3.5, fontface = "bold", color = "black") +
  theme_minimal() +
  labs(
    title = "Event Analysis: Okun's Law and 2020 Policy Distortions",
    subtitle = "Red point highlights the emergency wage-subsidy decoupling effect",
